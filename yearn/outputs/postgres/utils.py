...
def cache_address(address: str) -> Address:
    try:
        ...
        try:
            return Address(chainid=chainid,address=address,is_contract=is_contract(address))
...
def last_recorded_block(Entity: db.Entity) -> int:
    ...
    '''
    return db.select(f'select max(block) from {Entity._table_} where chainid = {chainid}')[0]
