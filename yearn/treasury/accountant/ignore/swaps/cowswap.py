    ...
    if "Trade" in tx._events:
        for trade in tx._events["Trade"]:
            if trade.address == YSWAPS and trade["owner"] in treasury.addresses and trade['buyToken'] not in SKIP_TOKENS:
                # buy side
                if tx.token == trade["buyToken"] and tx.to_address.address in treasury.addresses and tx.amount == tx.token.scale_value(trade['buyAmount']):
                    return True
                # sell side
                elif tx.token == trade["sellToken"] and tx.from_address == trade['owner'] and tx.amount == tx.token.scale_value(trade['sellAmount']):
