...
def is_yfi_cdp_withdrawal(tx: TreasuryTx) -> bool:
    if tx._symbol == 'YFI' and tx.to_address.address in treasury.addresses and 'flux' in tx._events:
...
def is_usdc_cdp_withdrawal(tx: TreasuryTx) -> bool:
    if tx._symbol == 'USDC' and tx.to_address.address in treasury.addresses and 'flux' in tx._events:
        for event in tx._events['flux']:
