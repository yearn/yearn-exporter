...
def get_token_bucket(token) -> str:
    ...
    if token in STABLECOINS or token in INTL_STABLECOINS or is_fixed_forex(token): # or token == '0x9ba60bA98413A60dB4C651D4afE5C937bbD8044B': # yla
...
def _unwrap_token(token) -> str:
    ...
    if is_curve_lp_token(token):
        pool_tokens = set(str(_unwrap_token(coin)) for coin in get_underlying_coins(token))
        return _pool_bucket(pool_tokens)
    if is_balancer_pool(token): # should only be YLA # TODO figure this out
        pool_tokens = set(str(_unwrap_token(coin)) for coin in Contract(token).getCurrentTokens())
        return _pool_bucket(pool_tokens)
