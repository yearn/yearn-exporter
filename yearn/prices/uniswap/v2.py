...
def _uni_pool(address: Address) -> Contract:
    try:
...
class UniswapV2:
    ...
    def pools(self) -> Dict[Address,Dict[Address,Address]]:
        ...
        if len(pairs) < all_pairs_len:
            ...
            token0s = fetch_multicall(*[[_uni_pool(pool), 'token0'] for pool in pools_your_node_couldnt_get])
            token1s = fetch_multicall(*[[_uni_pool(pool), 'token1'] for pool in pools_your_node_couldnt_get])
            additional_pools = {
